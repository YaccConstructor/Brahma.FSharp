...
module Helpers =
    ...
    type NormalizedFloatArray =
        ...
        static member Float32Type() =
            Arb.Default.Float32()
            |> Arb.toGen
            |> Gen.filter (fun v -> v < 100.f && v > -100.f)
            |> Gen.arrayOf
            |> Arb.fromGen
    ...
    let checkDefault<'a when 'a: equality and 'a: struct> context expected kernel =
        let actual =
            opencl {
                ...
                do!
                    runCommand kernel
                    <| fun kernelPrepare -> kernelPrepare <| Range1D(Settings.doubledWgSize, Settings.wgSize) <| result
...
let stressTest<'a when 'a: equality and 'a: struct> context (f: Expr<'a -> 'a>) size rawF (isEqual: 'a -> 'a -> bool) =
    ...
    let actual =
        opencl {
            ...
            do!
                runCommand kernel
                <| fun kernelPrepare -> kernelPrepare <| Range1D.CreateValid(size, Settings.wgSize) <| result
        ...
        fun (array: 'a[]) ->
            ...
            let actual () =
                opencl {
                    ...
                    do!
                        runCommand (kernel Unchecked.defaultof<'a>)
                        <| fun kernelPrepare ->
                            kernelPrepare
            ...
            array.Length <> 0 ==> lazy (actual () .=. expected ())
    )
...
let xchgTest<'a when 'a: equality and 'a: struct> context cmp value =
    ...
    let actual =
        opencl {
            ...
            do!
                runCommand kernel
                <| fun kernelPrepare -> kernelPrepare <| Range1D(localSize, localSize) <| buffer
