...
let inline private mapAndCreateTest name source expected =
    ...
    createMapTestAndCompareAsStrings Atomic.parse name <| map source <| map expected
...
let ClCellMapTests =
    [
        ...
        <| <@
            fun (first: ClCell<int>) (second: ClCell<int>) (third: ClCell<int>) ->
                Atomic.Fun.atomicCmpxchg (ref first.Value) second.Value third.Value
        @>
...
let ClArrayMapTests =
    [
        ...
        <| <@
            fun (first: ClArray<int>) (second: ClArray<int>) (third: ClArray<int>) ->
                Atomic.Fun.atomicCmpxchg (ref first.[0]) second.[0] third.[0]
        @>
...
let inline createTestWithException<'a when 'a :> exn> name =
    createTestWithException<Expr, 'a> (Atomic.parse >> ignore) name
...
let tests =
    testList "Atomic" [ localMapTests; ClArrayMapTests; ClCellMapTests; common ]
