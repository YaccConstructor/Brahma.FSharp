module Brahma.FSharp.Tests.Translator.ControlFlow.Tests

open Brahma.FSharp
open Expecto
open Brahma.FSharp.Tests.Translator.Common

let inline createTest translator name fileName quotation = test name {
    Helpers.checkCode translator quotation fileName fileName // TODO(remove out file)
}

let controlFlowTests translator = [
    let inline createTest name = createTest translator name

    <@ fun (range: Range1D) (buf: int clarray) -> if 0 = 2 then buf.[0] <- 1 @>
    ...
    <@ fun (range: Range1D) (buf: int clarray) ->
        while true do
            buf.[0] <- 1
    @>
    |> createTest "WHILE with single statement." "WHILE.with.complex.condition.cl"

    // TODO(paths in test. Race condition?)
    <@ fun (range: Range1D) (buf: int clarray) ->
