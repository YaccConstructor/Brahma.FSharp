...
module GettingWorkSizeTransformer =
    ...
    let rec go (expr: Expr) =
        match expr with
        | Patterns.Let(var, Patterns.PropertyGet(Some(Patterns.Var(TypeName Range1D_ _)), WorkSize(_, q), _), inExpr) ->
            Expr.Let(
                var,
                (match q with
        ...
        | Patterns.LetVar(Name "patternInput" _,
        ...
        | Patterns.LetVar(Name "patternInput" _,
                          ...
                          Patterns.Let(varZ,
                                       ...
                                       Patterns.Let(varY,
                                                    Patterns.TupleGet(Patterns.Var(Name "patternInput" _), 1),
                                                    Patterns.Let(varX,
                                                                 Patterns.TupleGet(Patterns.Var(Name "patternInput" _),
                                                                                   0),
                                                                 inExpr)))) ->
            Expr.Let(
                varX,
